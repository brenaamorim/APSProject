...
class MyListViewController: UITableViewController {
    ...
    private var selectedPlist = PlistNames.toWatch
    ...
    func fetchData() {
        paraAssistir = FilmRepository(with: PlistNames.toWatch.rawValue).readAll()
        assistidos = FilmRepository(with: PlistNames.watched.rawValue).readAll()
    ...
    @objc fileprivate func handleSegmentChange() {
        ...
        case 0:
            selectedPlist = PlistNames.toWatch
            fetchData()
...
//                didStartFilter(with: filteredStreamings)
            ...
            updateHeaderViewHeight(for: tableView.tableHeaderView)
        case 1:
            selectedPlist = PlistNames.watched
            fetchData()
...
extension MyListViewController {
        ...
        if let path = film.poster_path {
            ...
            if let _ = film.imageData {
                ...
                film.downloaded(from: url) { data in
                    film.imageData = data
                    if self.selectedPlist == PlistNames.toWatch {
    ...
    func deleteRowAt(with item: Int, by plistHost: String, to plistReceive: String) {
        ...
        switch plistHost {
        case PlistNames.toWatch.rawValue:
            FilmRepository(with: plistHost).delete(object: film)
            FilmRepository(with: plistReceive).create(object: film)
        case PlistNames.watched.rawValue:
            FilmRepository(with: plistHost).delete(object: film)
        default:
            break
        }
